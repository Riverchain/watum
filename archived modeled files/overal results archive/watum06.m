...
dddddd = input('Calc dt press enter \n read dt press r  inter now press n  ','s');
if strcmp( dddddd , 'r' ) == 1
    
    dt   = xlsread(filename,'Setting','B10:B10');

elseif strcmp( dddddd , 'n' ) == 1
    dt = input('dt? ');
xlswrite(filename,dt,'Setting','B10:B10');
else
...
% xlswrite ( filename, c_peak_num, 'peak_num' )
 ...
 % please do this: max(c1(5900/50,:)) max(c1((station meters)/(spatial step),:))